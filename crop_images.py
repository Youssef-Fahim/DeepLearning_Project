...
def crop_faces(plot_images=False, max_images_to_plot=5):
   ...
   # return good_cropped_img_file_names
    ...
    #fh = open(config.ORIGINAL_IMGS_INFO_FILE)
    #all_imgs_info = csv.reader(fh, delimiter = ',', quotechar = '"')
    print column_headers
    print all_imgs_info
    #print good_cropped_img_file_names
    cropped_imgs_info=[]
    #cropped_imgs_info = [l for l in all_imgs_info if l.split(',')[-1] in good_cropped_img_file_names]
    for l in all_imgs_info:
        print l.split(',')[-1]
        print l
        if len(l.split(',')[-1])==5:
            a=l.split(',')[-1][0:1]
            b=l.split(',')[-1][1:]
            renam_variable = "000" + str(a) + ".jpg"
            total_variable = renam_variable + str(b)
            #print "000" + str(a) + ".jpg"
            #print renam_variable
            if renam_variable in good_cropped_img_file_names:
                cropped_imgs_info.append(total_variable)
        if len(l.split(',')[-1])==6:
            a=l.split(',')[-1][0:2]
            b=l.split(',')[-1][2:]
            renam_variable = "00" + str(a) + ".jpg"
            total_variable = renam_variable + str(b)
            #print "00" + str(a) + ".jpg"
            #print renam_variable
            if renam_variable in good_cropped_img_file_names:
        ...
        if l in good_cropped_img_file_names:
            cropped_imgs_info.append(l)
    
    print good_cropped_img_file_names
    print cropped_imgs_info
